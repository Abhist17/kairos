...
def run_live_broker(
    collector, symbol, interval, poll_seconds, tv_alerts=False, telegram_alerts=False
):
    ...
    print(
        f"  Alerts:   {'TV ' if tv else ''}{'Telegram ' if tg else ''}{'terminal only' if not tv and not tg else ''}"
    )
...
def generate_pine(symbol):
    ...
    print("Copy-paste into TradingView Pine Editor to display signals on chart.\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tv-alerts", action="store_true", help="Send TradingView webhook alerts"
    )
    parser.add_argument("--telegram", action="store_true", help="Send Telegram alerts")
    parser.add_argument(
        "--pine", action="store_true", help="Generate Pine Script and exit"
    )
    ...
    if args.broker == "demo":
        ...
        if collector:
            run_live_broker(
                ...
                args.interval,
                args.poll,
                tv_alerts=args.tv_alerts,
                telegram_alerts=args.telegram,
            )
