...
from engine.core.enums import MarketRegime, EntryWindow
...
REGIME_CLR = {
    MarketRegime.TREND_EXPANSION: GRN,
    MarketRegime.TREND_EXHAUSTION: YEL,
    MarketRegime.COMPRESSION: CYN,
    MarketRegime.MEAN_REVERSION: MAG,
    MarketRegime.CHAOTIC: RED,
    MarketRegime.UNKNOWN: GRY,
}
...
def render(state: MarketState, oi_result, gamma_result, idx: int, total: int):
    ...
    print(
        f"{B}{CYN}║  KAIROS ENGINE — Real-Time Options Entry Intelligence{R}{' ' * 27}{CYN}║{R}"
    )
    print(
        f"{B}{CYN}╚══════════════════════════════════════════════════════════════════════════════════╝{R}"
    )
    ...
    print(f"\n  {DIM}── Features ──{R}")
    print(
        f"  Efficiency   {bar(rm.efficiency_ratio, 1.0, 25, GRN)} {rm.efficiency_ratio:.3f}"
    )
    print(
        f"  Persistence  {bar(rm.directional_persistence, 1.0, 25, GRN)} {rm.directional_persistence:.3f}"
    )
    print(
        f"  Entropy      {bar(rm.directional_entropy, 1.0, 25, YEL)} {rm.directional_entropy:.3f}"
    )
    print(
        f"  Vol Pctile   {bar(rm.volatility_percentile, 100, 25, RED)} {rm.volatility_percentile:.1f}%"
    )

    # Compression
    print(f"\n  {DIM}── Compression ──{R}")
    comp_flag = (
        f"{CYN}{B}■ COMPRESSED{R}" if cm.is_compressed else f"{GRY}□ not compressed{R}"
    )
    hl = f"{cm.compression_half_life:.0f}" if cm.compression_half_life < 99999 else "∞"
    print(
        f"  Score: {bar(cm.compression_score, 1.0, 20, CYN)} {cm.compression_score:.3f}  "
    ...
    print(f"\n  {DIM}── Structure ──{R}")
    print(
        f"  Score: {bar(st.structure_score, 1.0, 20, BLU)} {st.structure_score:.2f}  "
    ...
    iv_clr = (
        GRN
        if iv.state.value == "COMPRESSED"
        else (RED if "OVER" in iv.state.value else WHT)
    )
    print(f"\n  {DIM}── IV State ──{R}")
    print(
        f"  State: {iv_clr}{B}{iv.state.value}{R}  "
    ...
    print(
        f"  Call Gravity: {oi_result.call_gravity:.0f}  "
    ...
    print(
        f"  GEX: {gamma_result.total_gamma_exposure:+,.0f}  "
    ...
    print(
        f"  {eff_clr}{B}{'✓ EFFICIENT' if op.is_efficient else '✗ NOT EFFICIENT'}{R}  "
    ...
    print(f"\n  {DIM}── Pressure / Flow ──{R}")
    print(
        f"  Pressure: {bar(fl.pressure_score, 1.0, 20, YEL)} {fl.pressure_score:.3f}  "
    ...
    print(f"\n  {DIM}── Thesis ──{R}")
    print(
        f"  Primary: {B}{th.primary_bias.value}{R} ({th.primary_score:.3f})  "
    ...
    if state.entry_window == EntryWindow.OPEN:
        ...
        print(
            f"  {B}{GRN}║  ▶ ENTRY WINDOW OPEN                                        ║{R}"
        )
        print(
            f"  {B}{GRN}║  Estimated Window: ~{sm.estimated_window_seconds}s"
            f"{'':>{42 - len(str(sm.estimated_window_seconds))}}║{R}"
        )
        print(
            f"  {B}{GRN}║  Thesis Survival: {sm.thesis_survival_minutes:.0f} minutes"
...
def run_csv(path: str):
    ...
    for i in range(min_c, total, 3):
        ...
        state = pipeline.process(
            c, h, lo, o, v, timestamp=candles[i - 1].timestamp, symbol=candles[0].symbol
        )
