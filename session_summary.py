...
def main():
    ...
    try:
        ...
        print("  Run live.py first to generate data")
    ...
    try:
        ...
        if not trades:
            ...
            print("    Use the interactive trade logger to record trades")
        else:
            print(f"    Trades today: {len(trades)}")
            print("")
            print(
                f"    {'Time':<10s} {'Symbol':<8s} {'Strike':>8s} {'Type':<4s} "
            ...
            for t in trades:
                ...
                pnl_color = (
                    GRN if (t["pnl"] or 0) > 0 else (RED if (t["pnl"] or 0) < 0 else R)
                )
                exit_str = f"₹{t['exit_price']:.2f}" if t["exit_price"] else "—"
                ts = t["entry_time"][:8] if t["entry_time"] else "—"

                print(
                    f"    {ts:<10s} {t['symbol']:<8s} {t['strike']:>8.0f} {t['option_type']:<4s} "
        ...
        if t_stats["closed"] > 0:
            ...
            wr_color = (
                GRN
                if t_stats["win_rate"] > 55
                else (RED if t_stats["win_rate"] < 45 else YEL)
            )
            pnl_color = GRN if t_stats["total_pnl"] > 0 else RED
            print(f"    {B}All-time stats:{R}")
            print(
                f"      Closed: {t_stats['closed']}  "
